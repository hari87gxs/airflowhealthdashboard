...
async def health_check():
    ...
    """
    airflow_status = (
        "connected" if await airflow_client.test_connection() else "disconnected"
    )
...
async def get_domains(
    time_range: TimeRange = Query(
        TimeRange.HOURS_24, description="Time range for filtering DAG runs"
    ),
    force_refresh: bool = Query(
        False, description="Force refresh from Airflow, bypassing cache"
    ),
):
    """
    ...
    try:
        logger.info(
            f"GET /domains - time_range: {time_range.value}, force_refresh: {force_refresh}"
        )
        return await health_service.get_dashboard_data(
            time_range, force_refresh=force_refresh
        )
    except Exception as e:
        logger.error(f"Error fetching domains: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch domain data: {str(e)}"
        )
...
async def get_domain_detail(
    ...
    try:
        logger.info(
            f"GET /domains/{domain_tag} - time_range: {time_range.value}, force_refresh: {force_refresh}"
        )
        return await health_service.get_domain_detail(
            domain_tag, time_range, force_refresh=force_refresh
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch domain detail: {str(e)}"
        )


@router.get(
    "/domains/{domain_tag}/dags/{dag_id}/runs", response_model=List[DagRunSummary]
)
async def get_dag_runs(
    ...
    limit: int = Query(
        50, ge=1, le=100, description="Maximum number of runs to return"
    ),
):
    """
    ...
    try:
        logger.info(
            f"GET /domains/{domain_tag}/dags/{dag_id}/runs - time_range: {time_range.value}"
        )
        return await health_service.get_dag_runs(dag_id, time_range, limit)
    except Exception as e:
        logger.error(f"Error fetching DAG runs: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch DAG runs: {str(e)}"
        )
...
async def get_failure_analysis(
    time_range: TimeRange = Query(
        TimeRange.HOURS_24, description="Time range for failure analysis"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate failure analysis: {str(e)}"
        )
...
async def send_report_now(
    time_range: TimeRange = Query(
        TimeRange.HOURS_24, description="Time range for the report"
    ),
    include_ai_analysis: bool = Query(
        True, description="Include AI-powered failure analysis in the report"
    ),
):
    """
    ...
    try:
        logger.info(
            f"POST /reports/send - Sending manual report for {time_range.value}"
        )

        success = await scheduled_reporter.generate_and_send_report(
            time_range=time_range, include_ai_analysis=include_ai_analysis
        )
