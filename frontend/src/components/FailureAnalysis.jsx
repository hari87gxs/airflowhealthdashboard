...
function FailureAnalysis({ timeRange }) {
  ...
  console.log('FailureAnalysis render - analysis:', analysis, 'loading:', loading, 'error:', error); // Debug log
  ...
  const fetchAnalysis = async () => {
    try {
      console.log('Starting failure analysis fetch for timeRange:', timeRange); // Debug log
      setLoading(true);
      setError(null);
      const data = await api.getFailureAnalysis(timeRange);
      console.log('Failure analysis data received:', data); // Debug log
      setAnalysis(data);
    } catch (err) {
    ...
    return (
      <div className="bg-white rounded-lg shadow p-8">
        <div className="flex items-center justify-center">
          ...
          <span className="ml-4 text-gray-600">Loading DAG analysis...</span>
        </div>
      </div>
    );
  }
  ...
  if (!analysis && !loading && !error) {
    return (
      <div className="bg-blue-50 border border-blue-200 rounded-lg shadow p-8">
        <h2 className="text-xl font-bold text-blue-900 mb-4">🔍 DAG Health Analysis</h2>
        <p className="text-blue-700">Analysis data is being prepared in the background. Please refresh in a moment.</p>
        <button
          onClick={fetchAnalysis}
